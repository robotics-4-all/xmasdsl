...
from xmasdsl.m2t import objectToJSON
...
def range_obj_processor(range):
    ...
    if int(range.start) < 0 or int(range.end) > MAX_LEDS or int(range.start) > MAX_LEDS:
        raise_validation_error(
            range, f'Invalid range. Value is out of bounds: [0, {MAX_LEDS}]')
    pass
...
def random_range_obj_processor(rand_range):
    ...
    if (int(rand_range.percentage) > 100 or int(rand_range.percentage) < 0):
        raise_validation_error(
            rand_range, f'Invalid percentage. Value is out of bounds: [0, 100]')
    pass
...
def estimate_model_duration(model):
    _, duration = objectToJSON(model.program)

    return duration


def validate_model_duration(model, max_duration=MAX_DURATION):
    _, duration = objectToJSON(model.program)

    if duration > max_duration:
        raise_validation_error(
            model, f"Model duration is: {duration/1000}. It must be lower than {max_duration}.")
